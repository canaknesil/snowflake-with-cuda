...
class CUDAApplicationGenerator(ast.NodeVisitor):
	...
	def visit_StencilComponent(self, node):
		...
		str = "StencilComponent(\"" + node.name + "\", " + node.name + "__WEIGHTS__, " + dimAndsize[0] + ", " + dimAndsize[1] + ")"
		return str
...
class CUDACompiler(Compiler):
	...
	def _post_process(self, original, compiled, index_name, **kwargs):
		...
		subprocess.call("cat " + cudaPartsDir + "/extra-defs.cu.part >> cudaSrc/main.cu", shell=True)
		subprocess.call("cat " + cudaPartsDir + "/main-start.cu.part >> cudaSrc/main.cu", shell=True)
		...
		subprocess.call("cat " + cudaPartsDir + "/main-end.cu.part >> cudaSrc/main.cu", shell=True)
